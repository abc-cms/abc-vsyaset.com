			...
			if ($sql) {
				...
				$sql = rtrim($insert . $sql, ";");
					...
					elseif($this->mysql_version > 40101 && empty($last_charset)) { // Устанавливаем кодировку на случай если отсутствует CREATE TABLE
						mysqli_query($connect,"SET $this->restore_charset '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
						echo tpl_l("Установлена кодировка соединения `" . $this->restore_charset . "`.", C_WARNING);
						$last_charset = $this->restore_charset;
					}
					$insert = '';
				...
				if ($execute) {
					...
					mysqli_query($connect,$sql) or trigger_error ("Неправильный запрос.<BR>" . mysqli_error($connect), E_USER_ERROR);
						...
						$aff_rows += mysqli_affected_rows($connect);
					}
					$sql = '';
					$query_len = 0;
					$execute = 0;
...
<TR>
<FORM NAME=skb METHOD=POST ACTION="">
...
<div style="display:none"><SELECT NAME=db_backup>{$SK->vars['db_backup']}</SELECT></div>
